    ...
    async def batch_execute(
        ...
        for i in range(0, len(tasks), batch_size):
            ...
            try:
                ...
                log.error(f"Batch timeout after {timeout}s")
                raise
...
class BatchProcessor:
    ...
    def __init__(self, batch_size: int = 100, max_concurrent: int = 5):
        ...
        self.queue = asyncio.Queue() if hasattr(asyncio, 'Queue') else None
    ...
    async def process_batches(
        ...
        **kwargs
        ...
        for i in range(0, len(items), self.batch_size):
            ...
            log.info(f"[BatchProcessor] Processing batch {i//self.batch_size + 1} ({len(batch)} items)")
